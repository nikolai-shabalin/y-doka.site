...
Тесты делают код более прочным и живучим. Одновременно с этим тесты — это отличная документация, которая не врёт и не устаревает. Также тесты можно использовать, как инструмент при разработке программы.
...
- Не хватает выразительности и лаконичности — сравнивать значения и выбрасывать ошибки руками некруто.
...
Попробуем переписать тест нашей функции `add`, используя его:
...
Это частный случай регрессий. Обновление зависимостей не только фиксит баги и несёт новые фичи, но иногда и ломает логику работы. Если наш код протестирован, то такие ломающие обновления мы определим сразу.
...
Избавиться от ощущения ненужности тестов можно, если заранее закладывать время на них в план, а также считать их базовой гигиеной при работе. Часть команд и проектов сейчас не работают без тестов вовсе. Такие команды — это хорошая среда для того, чтобы привить себе тестирование до уровня автоматизма.
...
Грамотно организовать систему с фиктивным объектами сложно. Это требует навыков проектирования, знаний о хорошей архитектуре и опыта.
...
<figure>
  <img src="/assets/images/posts/js/testing/testing-pyramid.png" width="381" height="329">
  <figcaption>Пирамида тестирования: в основании Unit-тесты, чуть выше интеграционные, на вершине — End-to-End</figcaption>
</figure>
...
Как правило, бóльшая часть тестов в проекте — это модульные тесты. Их проще писать, для них не требуется *слишком сложной* структуры, они быстро проверяются и их можно запускать параллельно, потому что они не зависят друг от друга.

:::callout 💡

На юнит-тестах основана методолгия разработки через тестирование, TDD. Мы рассмотрим работу по TDD в отдельной статье.
...
Часто для интеграционного тестирования нужен отдельный фреймворк, который бы умел читать спецификации задач и проверять, что программа работает правильно. Важно, что в таком тестировании мы проверяем *не порядок вызова* частей программы, а именно *результат* их совместной работы.

Интеграционными тестами часто допроверяют связующие модули (так называемое middleware):
...
Такие тесты играют роль пользователей, которые ходят по приложению и выполняют разные задания. Для этих тестов тоже необходим фреймворк, который бы умел манипулировать приложением, как это делают люди.
...
- или проект точно не будет жить долго — предсказать такое сложно, но если мы уверены, что тест не успеют окупиться, ими можно пренебречь.

В остальных случаях тесты лучше начать писать с самого начала.
