...
Объектно-ориентированное программирование — парадигма, которая помогает спроектировать программу так, чтобы она верно отражала эти правила и была устойчивой к изменениям.
...
Суть инкапсуляции — объединить код, данные модуля и доступ к этим данным внутри этого модуля, исключить бесконтрольный доступ к ним извне.

Когда наш код организован так, что внутри модуля содержится лишь код, который занимается близкими задачами (всё про бюджет — в модуле Бюджета, а всё про историю — в модуле Истории) — у него [_высокая связность_](<https://ru.wikipedia.org/wiki/Связность_(программирование)>). Инкапсуляция помогает повысить связность.
...
А чтобы посмотреть, как эти принципы применимы на практике и какую приносят пользу, рекомендуем прочесть пост [«Рисуем деревья на canvas с помощью L-систем, TypeScript и ООП»](https://bespoyasov.ru/blog/generating-trees-on-canvas-using-typescript-and-oop/).
