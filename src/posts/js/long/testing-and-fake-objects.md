...
Когда мы пишем тесты, мы [проверяем предположения](/js/long/how-to-test-and-why/) о работе функций. Если тестируемая функция чистая, зависит только от переданных аргументов, то и проверка предположения будет простой:
...
Если можно не мóкать — лучше не мокать.
...
Следить за размером тестового кода и инфраструктуры помогает [TDD](/js/long/tdd/). По этой методологии мы сначала пишем тест, а только потом реализацию. Это помогает сразу проектировать API так, чтобы тест не оказался сложным.
